...
def blackpad_preprocessor(
    channel_stack: np.ndarray,
    params: dict
):
    """
    Preprocessors like this one, blackpad_preprocessor, are used
    on stacks_of_channels, at a minimum the 3 rgb channels and one mask channel,
    but sometimes many more channels for the aspect of multiple prediction tasks
    ...
    assert isinstance(channel_stack, np.ndarray)
    assert (
        channel_stack.shape[2] >= 4
    ), f"ERROR: The channel_stack should probably have at least 4 channels if you stacked rgb and just one mask, but it has only {channel_stack.shape[2]=} channels"
    ...
    padded_frame = np.zeros((desired_height, desired_width, num_channels), dtype = np.uint8)
    ...
    assert padded_frame.dtype == np.uint8
