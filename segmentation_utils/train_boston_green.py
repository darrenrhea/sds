...
find ~/celtics/400x400/ -type f -name '*.png' -delete
tmux new -s parallel_train
conda activate floor_not_floor
cd ~/r/segmentation_utils/
CUDA_VISIBLE_DEVICES=2,3 python -m fastai.launch train_boston_green.py
"""
...
list_of_annotated_images = get_list_of_annotated_images_from_several_directories(
    ...
    directories_to_gather_from_with_limits = [
        (
            Path(f"~/r/boston_multi/thomas").expanduser(),
            100000
        ),
        (
            Path(f"~/r/boston_multi/baynzo").expanduser(),
            100000
        ),
        (
            Path(f"~/r/boston_multi/priebe").expanduser(),
            100000
        ),
    ]
)
...
# lets resume training from the crazybake model:

num_epochs = 6000
model_path = Path("~/r/trained_models").expanduser()
model = unet_learner(dls, resnet34)
# model = model.load(Path("~/r/trained_models/celtics_400p_400p_res34_1e_3f_crazybake_full_res").expanduser())
model = model.load(Path("~/r/trained_models/green_uniform_celtics_400p_400p_res34_4e_29f_full_res").expanduser())
...
with model.distrib_ctx(sync_bn=False):  # if you don't put in this is will say some whack about batch norm
    for i in range(5, num_epochs + 1):
        model.fine_tune(1)
        model_name = f"green_uniform_celtics_400p_400p_res34_{i}e_{num_frames}f_full_res"
