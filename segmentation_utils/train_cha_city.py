"""
Move the (original, mask) pairs to be used to train to lam a la:
rsync -rP ~/cha_city_temp/ lam:~/cha_city_temp/
ssh lam
mkdir -p ~/cha_city_crops/400x400
Might want to delete previously created crops:
find ~/cha_city_crops/400x400 -type f -name '*.png' -delete

Since this tends to run for hours, we tend to do it in a tmux so it doesn't hang up:
tmux ls
tmux new -s parallel_train
conda activate floor_not_floor
cd ~/r/segmentation_utils/
ls -1 ~/cha_city_temp/*_nonfloor.png | wc -l
ls -1 ~/cha_city_temp/*.jpg | wc -l
# get the indices of the RTX 8000s
nvidia-smi
# make sure 0, 3 are the RTX 8000s, an confirm not too much GPU memory being used by other programs.
CUDA_VISIBLE_DEVICES=0,3 python -m fastai.launch train_cha_city.py
"""
...
list_of_annotated_images = get_list_of_annotated_images_from_several_directories(
    ...
    directories_to_gather_from_with_limits = [
        (
            Path(f"~/cha_city_temp").expanduser(),
            100000
        ),
    ]
)
...
croppings_dir = "~/cha_city_crops/400x400"
...
model = model.load(
    Path(
        "~/r/trained_models/wood_generic_400p_400p_res34_55e_178f_full_res"
    ).expanduser()
)
...
with model.distrib_ctx(sync_bn=False):  # if you don't put this here, it will say some whack about batch norm
    for i in range(1, num_epochs + 1):
        model.fine_tune(1)
        model_name = f"cha_city_floor_400p_400p_res34_{i}e_{num_frames}f_full_res"
