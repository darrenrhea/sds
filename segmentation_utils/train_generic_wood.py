...
ls -1 ~/approved_temp/*_nonwood.png | wc -l
ls -1 ~/approved_temp/*.jpg | wc -l
rsync -rP ~/approved_temp lam:~/

Might want to delete previously created crops.
mkdir -p ~/generic_wood_crops/400x400
find ~/generic_wood_crops/400x400 -type f -name '*.png' -delete
tmux ls
...
CUDA_VISIBLE_DEVICES=0,3 python -m fastai.launch train_generic_wood.py
"""
...
list_of_annotated_images = get_list_of_annotated_images_from_several_directories(
    ...
    directories_to_gather_from_with_limits = [
        (
            Path(f"~/approved_temp").expanduser(),
            100000
        ),
    ]
)
...
croppings_dir = "~/generic_wood_crops/400x400"
...
model = model.load(Path("~/r/trained_models/wood_generic_400p_400p_res34_noised_102e_704f_full_res").expanduser())

with model.distrib_ctx(sync_bn=False):  # if you don't put this here, it will say some whack about batch norm
    for i in range(103, num_epochs + 1):
        model.fine_tune(1)
        model_name = f"wood_generic_400p_400p_res34_noised_{i}e_{num_frames}f_full_res"
