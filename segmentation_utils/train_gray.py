...
# rm -rf ~/gray_gray_herring_bone_gray_parquet_temp
get_all_segmentation_annotations
ls -1 ~/gray_gray_herring_bone_gray_parquet_temp/*_nonwood.png | wc -l
ls -1 ~/gray_gray_herring_bone_gray_parquet_temp/*.jpg | wc -l
rsync -rP ~/gray_gray_herring_bone_gray_parquet_temp lam:~/

Might want to delete previously created crops.
mkdir -p ~/gray_crops/400x400
find ~/gray_crops/400x400 -type f -name '*.png' -delete
tmux ls
...
CUDA_VISIBLE_DEVICES=0,3 python -m fastai.launch train_gray.py
"""
...
list_of_annotated_images = get_list_of_annotated_images_from_several_directories(
    ...
    directories_to_gather_from_with_limits = [
        (
            Path(f"~/gray_fourth_temp").expanduser(),
            100000
        ),
    ]
)
...
croppings_dir = "~/gray_fourth_crops/400x400"
...
# model = model.load(
#     Path(
#         "~/r/trained_models/gray_400p_400p_res34_1e_41f_full_res"
#     ).expanduser()
# )
...
with model.distrib_ctx(sync_bn=False):  # if you don't put this here, it will say some whack about batch norm
    for i in range(1, num_epochs + 1):
        model.fine_tune(1)
        model_name = f"gray_400p_400p_res34_{i}e_{num_frames}f_half_res"
