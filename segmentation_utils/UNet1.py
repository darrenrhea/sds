...
class UNet1(nn.Module):
    def __init__(self, in_channels, out_channels, nn_input_height, nn_input_width):
        ...
        self.conv2 = self.contract_block(in_channels=32,
                                         ...
                                         kernel_size=3,
                                         padding=1)
        self.conv3 = self.contract_block(in_channels=64,
                                         out_channels=128,
                                         kernel_size=3,
                                         padding=1)

        self.upconv3 = self.expand_block(in_channels=128,
                                         out_channels=64,
                                         kernel_size=3,
                                         padding=1)
        self.upconv2 = self.expand_block(in_channels=64 * 2,
                                         out_channels=32,
                                         kernel_size=3,
                                         padding=1)
        self.upconv1 = self.expand_block(in_channels=32 * 2,
    ...
    def forward(self, x):  # used to say __call__
        ...
        assert conv2.shape == (bs, 64, nn_input_height // 4, nn_input_width // 4)  # 56 x 56

        conv3 = self.conv3(conv2)
        assert conv3.shape == (bs, 128, nn_input_height // 8, nn_input_width // 8)  # 28 x 28

        upconv3 = self.upconv3(conv3)
        assert upconv3.shape == (bs, 64, nn_input_height // 4, nn_input_width // 4)  # 56 x 56

        upconv2 = self.upconv2(torch.cat(tensors=[upconv3, conv2], dim=1))
        assert upconv2.shape == (bs, 32, nn_input_height // 2, nn_input_width // 2)  # 112 x 112
