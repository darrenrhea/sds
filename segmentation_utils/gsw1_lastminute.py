...
# CUDA_VISIBLE_DEVICES=2,3 python -m fastai.launch gsw1_lastminute.py
# started at 2:17
...
desired_mask_names = ["inbounds", "nonfloor"]
...
list_of_annotated_images = get_list_of_annotated_images_from_several_directories(
    ...
    directories_to_gather_from_with_limits = [
        (
            Path(f"~/r/final_gsw1/scorebug_final_segmentation/").expanduser(),
            10000
        ),
        (
            Path(f"~/r/final_gsw1/blackened/").expanduser(),
            10000
        ),
        (
            Path(f"~/r/philadelphia_76ers/nonfloor_segmentation/").expanduser(),
            10000
        )
    ]
)
...
dct = cut_this_many_interesting_subrectangles_from_annotated_image(
    annotated_image=list_of_annotated_images[2],  # which annotated_image to cut croppings from
    how_many_croppings_to_cut_out=1,
    crop_height=crop_height,
    crop_width=crop_width,
    desired_mask_names=desired_mask_names,
    mask_name_to_min_amount=dict(inbounds=100),
...
if regenerate_crops:
    cropped_hand_annotated_training_data = get_numpy_arrays_of_croppings_and_their_masks(
        ...
        crop_width=crop_width,
        desired_mask_names=desired_mask_names,
        mask_name_to_min_amount=dict(inbounds=1000),  # we are forcing croppings to have at least one pixel of mainrectangle mask on
        mask_name_to_max_amount=dict(),
        how_many_originals=len(list_of_annotated_images),
        how_many_crops_per_original=100,
        mask_encoding_type="alpha_channel_of_rgba"
    )
...
model = model.load(Path("~/r/trained_models/gsw_blackened_400p_400p_res34_13e_71f_full_res").expanduser())
...
with model.distrib_ctx(sync_bn=False):  # if you don't put in this is will say some whack about batch norm
    for i in range(14, 14 + 1):
        model.fine_tune(1)
        model_name = f"gsw_blackened_400p_400p_res34_{i}e_71f_full_res"
