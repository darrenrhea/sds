...
get_all_segmentation_annotations people_baseball busch_people
rm -rf ~/busch_people_temp
mkdir -p ~/busch_people_temp
cp ~/approved_temp/* ~/busch_people_temp/
ssh lam
rm -rf ~/busch_people_temp
rsync -rP ~/busch_people_temp lam:~/

ls -1 ~/busch_people_temp/*_nonfloor.png | wc -l
ls -1 ~/busch_people_temp/*.jpg | wc -l

ssh lam
Might want to delete previously created crops.
mkdir -p ~/busch_people_crops/400x400
find ~/busch_people_crops/400x400 -type f -name '*.png' -delete

# might want to resume from the latest model:
ls -lrt ~/r/trained_models/
...
nvidia-smi
# make sure 0, 2 are the RTX 8000s and there is enough memory

CUDA_VISIBLE_DEVICES=0,2 python -m fastai.launch train_baseball.py
"""
...
list_of_annotated_images = get_list_of_annotated_images_from_several_directories(
    ...
    directories_to_gather_from_with_limits = [
        (
            Path(f"~/busch_people_temp").expanduser(),
            100000
        ),
    ]
)
...
croppings_dir = "~/busch_people_crops/400x400"
...
if regenerate_crops:
    cropped_hand_annotated_training_data = get_numpy_arrays_of_croppings_and_their_masks(
        ...
        how_many_crops_per_original=50,
...
model = model.load(
    Path(
        "~/r/trained_models/busch_people_400p_400p_res34_46e_603f_full_res"
    ).expanduser()
)
...
with model.distrib_ctx(sync_bn=False):  # if you don't put this here, it will say some whack about batch norm
    for i in range(1, num_epochs + 1):
        model.fine_tune(1)
        model_name = f"busch_people_400p_400p_res34_{i}e_{num_frames}f_full_res"
