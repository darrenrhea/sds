...
CUDA_VISIBLE_DEVICES=0,2 python -m fastai.launch train_floor_euroleague.py
"""
...
model = model.load(
    Path(
        "~/r/trained_models/euroleague_floor_not_floor_400p_400p_res34_2e_370f_full_res"
    ).expanduser()
)

with model.distrib_ctx(sync_bn=False):  # if you don't put in this is will say some whack about batch norm
    for i in range(3, num_epochs + 1):
        model.fine_tune(1)
        model_name = f"euroleague_floor_not_floor_400p_400p_res34_{i}e_{num_frames}f_full_res"
