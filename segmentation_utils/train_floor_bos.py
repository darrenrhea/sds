...
find ~/bos/400x400/ -type f -name '*.png' -delete
tmux new -s bos_train
conda activate floor_not_floor
cd ~/r/segmentation_utils/
CUDA_VISIBLE_DEVICES=0,3 python -m fastai.launch train_floor_bos.py
"""
...
# celtics_400p_400p_res34_1e_94f_full_res
model = model.load(
    Path(
        "~/r/trained_models/bos_floor_not_floor_400p_400p_res34_30e_58f_full_res"
    ).expanduser()
)

with model.distrib_ctx(sync_bn=False):  # if you don't put in this is will say some whack about batch norm
    for i in range(31, num_epochs + 1):
        model.fine_tune(1)
        model_name = f"bos_floor_not_floor_400p_400p_res34_{i}e_{num_frames}f_full_res"
