...
# time python run_stitcher_downsampled_batch_frames_bw.py gsw1 147000 147031 0 fastai_bw_280p_320p_res34_32e_10f fastai_bw_resnet34_280_320 resnet34 > out34_fp16_bs45_downsampled_bw.txt
# time python run_stitcher_downsampled_batch_frames_bw.py ind1 154000 154999 1 fastai_bw_280p_320p_res34_32e_10f fastai_bw_resnet34_280_320 resnet34 > out34_fp16_bs45_downsampled_bw.txt
# time python run_stitcher_downsampled_batch_frames_bw.py curry 850 20750 50 1 fastai_bw_280p_320p_res34_32e_10f fastai_bw_resnet34_280_320 resnet34 > out34_fp16_bs45_downsampled_bw.txt
...
assert video_name in ["gsw1", "ind1", "curry"]
...
which_gpu = int(sys.argv[5])
assert which_gpu in [0, 1, 2, 3], "gpu id must be 0 or 1, see nvidia-smi"
model_name = sys.argv[6]
assert model_name in ["fastai_27e_50f", "fastai_24e_66f", "fastai_26e_66f", "fastai_29e_66f", "fastai_30e_66f", "fastai_resnet18_8e_10f", "fastai_resnet18_224p_32e_10f", "fastai_224p_res34_32e_10f", "fastai_280p_320p_res34_24e_10f", "fastai_bw_280p_320p_res34_32e_10f"]
masking_attempt_id = sys.argv[7]
# assert masking_attempt_id in ["final_bw"]
assert masking_attempt_id in ["fastai_8_bw", "fastai_9_bw", "temp", "test_stride", "fastai_resnet18", "fastai_resnet34_224", "fastai_resnet34_224_downsampled", "fastai_resnet18_224", "fastai_resnet34_280_320", "fastai_bw_resnet34_280_320"]

save_color_information_into_masks = True  # WARNING: massive speed and storage space implications!
...
torch_device = my_pytorch_utils.get_the_correct_gpu("A5000", which_copy=which_gpu)
...
num_images_scored = 0
assert (last_frame_index - first_frame_index + 50) % 50 == 0, "total number of frames needs to be an integer"
num_frames = (last_frame_index - first_frame_index + 50)//50
...
for frame_index in range(first_frame_index, last_frame_index + 1, step_size):
    ...
    # smaller_cv2 = cv2.resize(img_cv2, (j_max, i_max))
    # gray_cv2 = cv2.cvtColor(smaller_cv2, cv2.COLOR_BGR2GRAY)
    # hwc_np_uint8 = np.array(gray_cv2)
    ...
    # original_image_array[frame_index - first_frame_index, :, :, :] = smaller_cv2
    print(f"frame index {(frame_index - first_frame_index)//step_size}")
    original_image_array[(frame_index - first_frame_index)//step_size, :, :, :] = smaller_pil
    gray_pil = PIL.ImageOps.grayscale(smaller_pil)

    # smaller_pil.save("resized_image.png")
 
    # hwc_np_uint8 = np.array(smaller_pil)

    hwc_np_uint8 = np.array(gray_pil)

    channel_change_start = time.time()
    hwc_np_uint8 = hwc_np_uint8[:, :, None] * np.ones(3)[None, None, :]
    ...
    with torch.cuda.amp.autocast():
        for i in range(num_batches):
            ...
            for patch_index in range(batch_start_index, min(batch_end_index, tot_num_patches)):
                ...
                frame_number = (batch_number_to_frame_index_and_patch[patch_index][0] - first_frame_index)//step_size
...
for frame_index in range(binary_prediction.shape[0]):
    ...
    if (save_color_information_into_masks):
        ...
        frame_index = step_size*frame_index + first_frame_index
