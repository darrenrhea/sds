...
# rm -rf ~/wood_light_normal_temp
get_all_segmentation_annotations
ls -1 ~/wood_light_normal_temp/*_nonwood.png | wc -l
ls -1 ~/wood_light_normal_temp/*.jpg | wc -l
rsync -rP ~/wood_light_normal_temp lam:~/

Might want to delete previously created crops.
mkdir -p ~/wood_light_normal_crops/400x400
find ~/wood_light_normal_crops/400x400 -type f -name '*.png' -delete
tmux ls
...
# put the other GPU back into lam:
CUDA_VISIBLE_DEVICES=0,3 python -m fastai.launch train_wood_light_normal.py
"""
...
list_of_annotated_images = get_list_of_annotated_images_from_several_directories(
    ...
    directories_to_gather_from_with_limits = [
        (
            Path(f"~/wood_light_normal_temp").expanduser(),
            100000
        ),
    ]
)
...
croppings_dir = "~/wood_light_normal_crops/400x400"
...
model = model.load(
    Path(
        "~/r/trained_models/wood_light_normal_400p_400p_res34_36e_423f_full_res"
    ).expanduser()
)

with model.distrib_ctx(sync_bn=False):  # if you don't put this here, it will say some whack about batch norm
    for i in range(37, num_epochs + 1):
        model.fine_tune(1)
        model_name = f"wood_light_normal_400p_400p_res34_{i}e_{num_frames}f_full_res"
