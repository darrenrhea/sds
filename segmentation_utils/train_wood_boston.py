...
CUDA_VISIBLE_DEVICES=0,3 python -m fastai.launch train_wood_boston.py
"""
...
list_of_annotated_images = get_list_of_annotated_images_from_several_directories(
    ...
    directories_to_gather_from_with_limits = [
        (
            Path(f"~/r/wood_boston/luna").expanduser(),
            100000
        ),
        (
            Path(f"~/r/wood_boston/grace").expanduser(),
            100000
        )
    ]
)
...
with model.distrib_ctx(sync_bn=False):  # if you don't put in this is will say some whack about batch norm
    for i in range(1, num_epochs + 1):
        model.fine_tune(1)
        model_name = f"celtics_wood_not_wood_400p_400p_res34_{i}e_{num_frames}f_full_res"
