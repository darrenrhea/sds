...
mkdir -p ~/summer_league_crops/400x400/
find ~/summer_league_crops/400x400/ -type f -name '*.png' -delete
tmux new -s parallel_train
conda activate floor_not_floor
cd ~/r/segmentation_utils/
ls -1 ~/starters | wc -l
CUDA_VISIBLE_DEVICES=2,3 python -m fastai.launch train_wood_summer_league.py
"""
...
croppings_dir = "~/summer_league_crops/400x400"
...
model = model.load(Path("~/r/trained_models/wood_summer_league_400p_400p_res34_60e_28f_full_res").expanduser())

with model.distrib_ctx(sync_bn=False):  # if you don't put this here, it will say some whack about batch norm
    for i in range(65, num_epochs + 1):
        model.fine_tune(1)
        model_name = f"wood_summer_league_400p_400p_res34_{i}e_{num_frames}f_full_res"
