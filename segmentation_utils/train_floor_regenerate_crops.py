...
# rm -rf ~/den_core_floor_temp
get_all_segmentation_annotations
ls -1 ~/den_core_floor_temp/*_nonfloor.png | wc -l
ls -1 ~/den_core_floor_temp/*.jpg | wc -l
rsync -rP ~/den_core_floor_temp lam:~/

Might want to delete previously created crops.
mkdir -p ~/den_core_floor_crops/400x400
find ~/den_core_floor_crops/400x400 -type f -name '*.png' -delete
tmux ls
...
CUDA_VISIBLE_DEVICES=0,2 python -m fastai.launch train_floor.py
"""
...
list_of_annotated_images = get_list_of_annotated_images_from_several_directories(
    ...
    directories_to_gather_from_with_limits = [
        (
            Path(f"~/lal_core_floor_temp").expanduser(),
            100000
        ),
    ]
)
...
croppings_dir = "~/lal_core_floor_crops/400x400"
...
if regenerate_crops:
    cropped_hand_annotated_training_data = get_numpy_arrays_of_croppings_and_their_masks(
        ...
        how_many_crops_per_original=2000,
...
# model = model.load(
#     Path(
#         "~/r/trained_models/bos_floor_not_floor_400p_400p_res34_62e_61f_full_res"
#     ).expanduser()
# )
...
with model.distrib_ctx(sync_bn=False):  # if you don't put this here, it will say some whack about batch norm
    for i in range(1, num_epochs + 1):
        model.fine_tune(1)
        model_name = f"lal_core_floor_400p_400p_res34_{i}e_{num_frames}f_full_res"
