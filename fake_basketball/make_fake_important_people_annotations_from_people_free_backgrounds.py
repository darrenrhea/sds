from get_camera_posed_people_free_backgrounds import (
...
def make_fake_important_people_annotations_from_people_free_backgrounds(
    context_id: str
):
    """
    This makes a lot of fake important people convention annotations
    via first pasting the floor texture according to a random camera pose found
    in an actual track, and then pasting multiple cutouts on top of that.
    via pasting multiple cutouts onto each background annotation,
    and does that multiple times.
    """

    shared_dir = get_the_large_capacity_shared_directory()
    fakes_dir = shared_dir / f"fake_{context_id}_important_people_annotations"
    assert (
        not fakes_dir.is_dir()
    ), f"{fakes_dir=} already exists.  This is usually an error"
    fakes_dir.mkdir(exist_ok=True, parents=False)
   

    # You must already have fake background annotations in a folder:
    actual_annotations = get_camera_posed_people_free_backgrounds(
        subdir_name="munich_people_free_backgrounds"
    )

    # actual_annotations = actual_annotations[:2]
    # You have to choose how many cutouts of each kind to paste onto an actual annotation to make a fake annotation:
    cutouts = get_cutouts(
        context_id=context_id,
        diminish_for_debugging=True,
    )
    ...
    cutout_kind_to_num_cutouts_to_paste = dict(
     ...
     # choose what albumentations augmentation to use, per kind of cutout.
    ...
    for actual_annotation in actual_annotations:  # pick an actual/read segmentation annotation to add cutouts to:
        ...
        actual_annotation_rgba_np_u8 = open_as_rgba_hwc_np_u8(
            image_path=actual_annotation_original_file_path,
        )
        # this is a background image, so everything initially is background:
        actual_annotation_rgba_np_u8[:, :, 3] = 0
        ...
        for i in range(num_fakes_per_actual_annotation):
            ...
            prii_verbose = True
            if prii_verbose:
                print("The fake datapoint is these two images:")
