...
def write_hw_np_f16_to_16_bit_grayscale_png(
    hw_np_f16: np.ndarray,
    ...
    * (values outside of this range will be clipped to 0 or 1)
    ...
    """
    assert (
        isinstance(hw_np_f16, np.ndarray)
    ), f"ERROR: hw_np_f16 must be a numpy array, not {type(hw_np_f16)=}"

    assert (
        hw_np_f16.ndim == 2
    ), f"ERROR: hw_np_f16 must have 2 dimensions, but instead {hw_np_f16.ndim=}"

    assert (
        hw_np_f16.dtype == np.float16
    ), f"hw_np_f16 must have dtype float32, not {hw_np_f16.dtype}"
    ...
    hw_np_u16 = np.round(
        hw_np_f16.astype(np.float32) * 65535.0
