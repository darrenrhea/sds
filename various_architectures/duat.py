...
class DuAT(nn.Module):
    ...
    def forward(self, x, train = True, normalize = False):
        # backbone
        pvt = self.backbone(x)
        c1, c2, c3, c4 = pvt
        n, _, h, w = c4.shape
        ...
        if train:
            ...
            res = F.interpolate(output + output2 , size=x.shape[2:], mode='bilinear', align_corners=False)
...
if __name__ == '__main__':
    ...
    height, width = 1080 // 2, 1920 // 4
    input = torch.randn(batch, chans, height, width).to(device)
    ...
    for i in range(100):
        ...
        pred_semantic = model(input)
        torch.cuda.synchronize()
        dt = time.time() - start_time
        print(f'{dt * 1000:.2f}ms')
        if i >= 10:
            time_count.append(dt * 1000.0)

    print(f'mean inference time: {np.mean(time_count):.2f}')
