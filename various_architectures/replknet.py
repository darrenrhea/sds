...
class RepLKNet(nn.Module):
    ...
    def __init__(self, large_kernel_sizes, layers, channels, drop_path_rate, small_kernel,
        ...
        # FX: stem channels = 128
        if include_stem:
            self.channel_info = [128] + channels
        else:
    ...
    def forward_features(self, x):
        x = self.stem[0](x)        
        for i, stem_layer in enumerate(self.stem[1:]):
            if self.use_checkpoint:
        ...
        if self._out_indices is None:
            ...
            for stage_idx in range(self.num_stages):
                x = self.stages[stage_idx](x)
                if stage_idx in self._out_indices:
    ...
    def forward(self, x):
        ...
        if self._out_indices:
            #print('repl features', len(x), [xx.shape for xx in x])
            return x
        else:
            x = self.norm(x)
            x = self.avgpool(x)
...
def create_RepLKNet31B(drop_path_rate=0.3, num_classes=1000, use_checkpoint=True, small_kernel_merged=False, out_indices=None, include_stem = False):
    return RepLKNet(large_kernel_sizes=[31,29,27,13], layers=[2,2,18,2], channels=[128,256,512,1024],
                    ...
                    small_kernel_merged=small_kernel_merged, out_indices = out_indices, include_stem=include_stem)


def create_RepLKNet31L(drop_path_rate=0.3, num_classes=1000, use_checkpoint=True, small_kernel_merged=False, out_indices=None, include_stem = False):
    return RepLKNet(large_kernel_sizes=[31,29,27,13], layers=[2,2,18,2], channels=[192,384,768,1536],
                    drop_path_rate=drop_path_rate, small_kernel=5, num_classes=num_classes, use_checkpoint=use_checkpoint,
                    small_kernel_merged=small_kernel_merged, out_indices = out_indices, include_stem=include_stem)

def create_RepLKNetXL(drop_path_rate=0.3, num_classes=1000, use_checkpoint=True, small_kernel_merged=False, out_indices=None, include_stem = False):
...
class RepLKNetEncoder(nn.Module):
    def __init__(self, name, pretrained=True, in_channels=3, depth=5, output_stride=32, include_stem = False):
        super().__init__()

        MODEL_LUT = {
            'b': create_RepLKNet31B,
            'l': create_RepLKNet31L,
            'xl': create_RepLKNetXL,
        }
        ...
        kwargs = dict(
            ...
            #num_classes=0, 
            num_classes=None,
            out_indices=range(3 + int(include_stem)),
        ...
        self.model = MODEL_LUT[name](**kwargs)
        #model.structural_reparam()

        # TODO: adapt
        # state_dict = torch.load('pretrained/RepLKNet-31B_ImageNet-1K_384.pth')
        # keys = list(state_dict.keys())
        # for k in keys:
        #     if k[:5] == 'norm.' or k[:5] == 'head.':
        #         del state_dict[k]
...
if __name__ == '__main__':
    model = create_RepLKNet31B(small_kernel_merged=False, out_indices=range(5), num_classes=None)
    model.eval()
    print('------------------- training-time model -------------')
    #print(model)
    x = torch.randn(2, 3, 224, 224)
    origin_y = model(x)
    print('out size', len(origin_y), [x.shape for x in origin_y])
    model.structural_reparam()
