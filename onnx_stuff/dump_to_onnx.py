...
assert opt.onnx is not None, "You must specify an output file path for the onnx model"
onnx_out_file_path = Path(opt.onnx).resolve()
assert onnx_out_file_path.suffix == ".onnx", f"{onnx_out_file_path} should end with .onnx"
assert onnx_out_file_path.parent.is_dir(), f"{onnx_out_file_path.parent} is not a directory. We don't make directories for you."

# OPSET 17 makes ONNX v8 appear in Netron
...
if fn_input.endswith(".json") or fn_input.endswith(".json5"):
    ...
    list_of_input_and_output_file_paths = get_list_of_input_and_output_file_paths_from_json_file_path(
        json_file_path=Path(fn_input).resolve(),
...
onnx_dumper(
    ...
    onnx_out_file_path=onnx_out_file_path,
