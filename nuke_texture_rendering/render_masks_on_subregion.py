...
def render_masks_on_subregion(
    ...
    """
    This is rendering textured quads, but the grayscale version.
    given flat masks stuffed into 3D, "render" them at the specified pixel locations.
    ...
    This function will return a list of mask values for those pixel locations.
    ...
    for ad_placement_descriptor in ad_placement_descriptors:
        ...
        assert ad_height > 0
        texture_hw_np_f32 = ad_placement_descriptor.texture_hw_np_f32
        assert (
            texture_hw_np_f32.ndim == 2
        ), f"texture_hw_np_f32 should br grayscale not {texture_hw_np_f32.shape}"
    ...
    texture_int_heights = np.array(
        [ad_placement_descriptor.texture_hw_np_f32.shape[0] for ad_placement_descriptor in ad_placement_descriptors]
    )
    
    texture_int_widths = np.array(
        [ad_placement_descriptor.texture_hw_np_f32.shape[1] for ad_placement_descriptor in ad_placement_descriptors]
    )
    ...
    # If the ray did not hit any ad, we will render it as white.
    mask_values_f32 = np.zeros(
        shape=(ijs.shape[0], ),
        dtype=np.float32
    ) + 255

    for ad_index in range(0, num_ads):
        texture_hw_np_f32 = ad_placement_descriptors[ad_index].texture_hw_np_f32
        i_s = i_within_ad_raster[which_ad == ad_index]
        j_s = j_within_ad_raster[which_ad == ad_index]
        mask_values_f32[which_ad == ad_index] = map_coordinates(
            input=texture_hw_np_f32[:, :],
            coordinates=[i_s, j_s],
            order=1,
            cval=255,
        )
    
    mask_values_u8 = np.round(mask_values_f32).clip(0, 255).astype(np.uint8)
    return mask_values_u8
